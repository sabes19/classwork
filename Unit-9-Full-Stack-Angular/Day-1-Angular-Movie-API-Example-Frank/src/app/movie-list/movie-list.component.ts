...
async ngOnInit() {
 ...
 //  2. = - take with the method returns and assign it to (store it in)
 //  3. this.moviesList is assigned the data from step 2
  this.moviesList = await this.movieService.getMoviesList(); // Initialize our moviesList from service
}

deleteMovie(aMovie :MoviesInfo) {
  this.movieService.deleteMovie(aMovie)
}
